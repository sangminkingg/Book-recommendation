...
-사사용자가 특정 도서를 검색하거나 조회한 경우, 해당 도서와 유사한 도서들 중에서 상위 10위에 해당하는 도서들을 추천해줄 수 있다. 
""")
import streamlit as st
import pandas as pd
...
- 해당 도서를 추천목록으로 제시한다면 이용 후기가 많은 도서들 중에서 상대적으로 인기가 높은 도서들을 추천해주기 때문에, 이용자들이 만족할 확률이 높아짐
- 온오프라인 서점에 적용시 이용 후기 개수가 충분하지 않은 새로운 도서를 추천해줄 때는 이용자들이 도서의 평균 평점을 보고 구매 이용 여부를 결정할 수 있으므로, 
도서의 초기 구매률을 증가시키는데에도 도움이 될 수 있다.
""")
import streamlit as st
import pandas as pd
import matplotlib.pyplot as plt
...
- 출판연도별 도서 개수는 도서시장의 흐름과 트렌드를 파악할 수 있는 중요한 지표
- 특정 연도에 도서가 많이 출간된다는 것은 해당 연도의 도서시장이 활발하다는 것을 의미. 그 해에 인기있는 특정 주제나 장르의 도서가 많이 출간된 것일 수 있다
- 특정 연도에 출간된 도서 중에서 많은 인기를 끌었던 도서들을 추천해주는 도서추천 시스템 구현가능
""")
